...
while True:
    ...
    try:
        ...
        break
    #skip if some other problem less than 5 times
    ...
    print(url,len(text))
    count = count + 1
    #if we get back data tolerate only 5 failures
    ...
    if len(x) == 1 :
        ...
        #regular expression that pulls out an email address - other format
        x = re.findall('\nFrom: (\S+@\S+)\n', hdr)
        if len(x) == 1 :
            email = x[0];
            email = email.strip().lower()
            email = email.replace("<","")
    #looking for a data using regular expressions
    date = None
    y = re.findall('\Date: .*, (.*)\n', hdr)
    if len(y) == 1 :
        tdate = y[0]
        ...
        try:
            ...
            if fail > 5 : break
            continue
    #looking for the subject line using regular expressions
    subject = None
